...
/**
 * Gestiona las entradas del teclado en una ventana GLFW y administra el estado
 * de las teclas.
 */
...
public enum KeyHandler {
     ...
     * izquierda, derecha).
     */
    private static Set<Integer> MOVEMENT_KEYS = Set.of(Key.UP.getKeyCode(), Key.DOWN.getKeyCode(),
            Key.LEFT.getKeyCode(), Key.RIGHT.getKeyCode());
    ...
    /**
     * Retorna la tecla de movimiento efectiva basada en el estado actual del
     * movimiento. Si el movimiento temporal esta activo,
     * retorna la tecla temporal; de lo contrario, retorna la tecla base de
     * movimiento.
     *
     * @return El codigo de la tecla de movimiento efectiva. Puede ser la tecla base
     *         o la temporal, dependiendo de si el
    ...
    /**
     * Actualiza los estados de las teclas en el sistema, marcando como no activas
     * las banderas relacionadas con eventos de "tecla
     * recien presionada" y "tecla recien liberada". Adicionalmente, se asegura de
     * validar y sincronizar las teclas de movimiento
     * activas.
     */
    ...
    /**
     * Actualiza el estado de una tecla especifica, marcando si ha sido presionada o
     * liberada, y gestionando las banderas
     * relacionadas con eventos como "tecla recien presionada" o "tecla recien
     * liberada". Tambien sincroniza el estado de la tecla
     * con ImGui.
     *
    ...
    public static void updateMovementKeys() {
        MOVEMENT_KEYS = Set.of(Key.UP.getKeyCode(), Key.DOWN.getKeyCode(), Key.LEFT.getKeyCode(),
                Key.RIGHT.getKeyCode());
    }

    /**
     * Gestiona la presion o liberacion de una tecla de movimiento. Actualiza las
     * variables internas que reflejan las teclas de
     * movimiento activas, diferenciando entre los estados de presion y liberacion.
     * Si la tecla es presionada, se considera como
     * ultima tecla de movimiento registrada y se procesa su activacion; si se
     * libera, se realiza el manejo correspondiente para
     * actualizar el estado.
     *
     * @param key       codigo de la tecla que se esta gestionando
     * @param isPressed indica si la tecla esta siendo presionada (true) o liberada
     *                  (false)
     */
    ...
    /**
     * Gestiona la accion de presionar una tecla de movimiento. Actualiza las
     * variables internas para reflejar el estado de las
     * teclas base y temporal de movimiento, asegurando la coherencia en casos donde
     * las teclas oprimidas entran en conflicto (por
    ...
    private static void handleMovementPress(int key) {
        if (baseMovementKey == -1)
            baseMovementKey = key;
        else if (baseMovementKey != key) {
            if (isOpposite(baseMovementKey, key))
                temporaryMovementKey = key;
            else {
                baseMovementKey = key;
    ...
    /**
     * Gestiona la liberacion de una tecla de movimiento y actualiza las variables
     * internas de las teclas base y temporal. Si la
     * tecla liberada coincide con {@code temporaryMovementKey}, se restablece su
     * valor a {@code -1}. Si la tecla liberada
     * coincide con {@code baseMovementKey}, se reasigna la tecla base utilizando
     * {@code temporaryMovementKey} si este es valido,
    ...
    /**
     * Valida y actualiza las teclas de movimiento actualmente activas, asegurando
     * consistencia entre el estado de las teclas y
     * las variables correspondientes.
     * <ul>
     * <li>Si la tecla de movimiento base ({@code baseMovementKey}) ya no esta
     * activa o no esta presionada, se asigna una nueva
     * tecla base utilizando el metodo {@link #findNewBaseKey()}.
     * <li>Si la tecla de movimiento temporal ({@code temporaryMovementKey}) deja de
     * estar presionada, se restablece su valor a
     * -1, indicando la ausencia de una tecla temporal activa.
     * </ul>
     * Esta validacion es esencial para gestionar correctamente la persistencia de
     * las teclas de movimiento en un contexto que
     * permite multiples entradas.
     */
    private static void validateMovementKeys() {
        if (baseMovementKey != -1 && !keyPressed[baseMovementKey])
            baseMovementKey = findNewBaseKey();
        if (temporaryMovementKey != -1 && !keyPressed[temporaryMovementKey])
            temporaryMovementKey = -1;
    }

    /**
     * Busca y retorna el codigo de la primera tecla de movimiento activa, evaluando
     * las teclas definidas en
     * {@code MOVEMENT_KEYS}. Una tecla se considera activa si su estado actual en
     * el arreglo {@code keyPressed} es {@code true}.
     *
     * @return el codigo de la primera tecla de movimiento activa, o {@code -1} si
     *         ninguna tecla de movimiento esta activa
     */
    ...
    /**
     * Determina si el movimiento temporal esta activo evaluando el estado de las
     * teclas asociadas al movimiento base y movimiento
     * temporal. Esta activo cuando ambas teclas, la de movimiento temporal y la de
     * movimiento base, estan presionadas, y ademas
     * la tecla de movimiento temporal no tiene un valor predeterminado (-1).
     *
     * @return {@code true} si el movimiento temporal esta activo; {@code false} en
     *         caso contrario
     */
     ...
     * @return {@code true} si las teclas representan direcciones opuestas;
    ...
    private static boolean isOpposite(int key1, int key2) {
        ...
        return (key1 == up && key2 == down) || (key1 == down && key2 == up) || (key1 == left && key2 == right)
    ...
    private static boolean isModifierPressed(int leftKey, int rightKey) {
        return keyPressed[leftKey] || keyPressed[rightKey];
    }

    /**
     * Resetea todos los estados de entrada.
     * Util tras cerrar dialogos bloqueantes (Swing) para evitar "teclas pegadas".
     */
