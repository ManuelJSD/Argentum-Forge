    ...
    public void render() {
        ...
        if (ImGui.begin(getLabel("title", "--- PANEL DE FOTOGRAFÍA ---"), flags)) {
            ...
            if (ImGui.beginTabBar("PhotoTabs")) {
                ...
                // TAB 1: LUZ Y COLOR
                if (ImGui.beginTabItem(getLabel("tab_light", "Luz y Color"))) {
                    ImGui.dummy(0, 5);

                    float[] exp = { settings.getPhotoExposure() };
                    if (ImGui.sliderFloat("Exposición", exp, 0.5f, 2.0f))
                        settings.setPhotoExposure(exp[0]);

                    float[] con = { settings.getPhotoContrast() };
                    if (ImGui.sliderFloat("Contraste", con, 0.5f, 2.0f))
                        settings.setPhotoContrast(con[0]);

                    float[] sat = { settings.getPhotoSaturation() };
                    if (ImGui.sliderFloat("Saturación", sat, 0.0f, 2.0f))
                        settings.setPhotoSaturation(sat[0]);

                    ImGui.separator();
                    ImGui.text("Filtro:");
                    String[] filters = { "Ninguno", "Escala de Grises", "Sepia", "Vintage", "Cálido", "Frío" };
                ...
                if (ImGui.beginTabItem(getLabel("tab_cam", "Enfoque y Cámara"))) {
                    ImGui.dummy(0, 5);

                    float[] zoom = { settings.getPhotoZoom() };
                    if (ImGui.sliderFloat("Zoom Óptico", zoom, 1.0f, 5.0f))
                        settings.setPhotoZoom(zoom[0]);

                    ImGui.separator();
                    if (ImGui.checkbox("Profundidad de Campo", settings.isPhotoDoF()))
                        settings.setPhotoDoF(!settings.isPhotoDoF());
                    if (settings.isPhotoDoF()) {
                        float[] foc = { settings.getDofFocus() };
                        if (ImGui.sliderFloat("Punto Enfoque", foc, 0.0f, 1.0f))
                            settings.setDofFocus(foc[0]);
                        float[] ran = { settings.getDofRange() };
                        if (ImGui.sliderFloat("Rango Enfoque", ran, 0.0f, 1.0f))
                            settings.setDofRange(ran[0]);
                    }

                    ImGui.separator();
                    if (ImGui.checkbox("Pausar Tiempo", settings.isPhotoTimeStop()))
                        settings.setPhotoTimeStop(!settings.isPhotoTimeStop());
                    ImGui.textColored(ImGui.getColorU32(0.7f, 0.7f, 0.7f, 1.0f), "Congela animaciones y NPCs");

                    ImGui.endTabItem();
                }

                // TAB 4: SOMBRAS
                if (ImGui.beginTabItem(getLabel("tab_shadows", "Sombras"))) {
                    ImGui.dummy(0, 5);
                    if (ImGui.checkbox("Sombras de Objetos", settings.isPhotoShadows()))
                        settings.setPhotoShadows(!settings.isPhotoShadows());
                    if (settings.isPhotoShadows()) {
                        ImGui.indent(20);
                        if (ImGui.checkbox("Sombras Suaves (HQ)", settings.isPhotoSoftShadows()))
            ...
            if (ImGui.button(getLabel("take_photo", "HACER FOTO (F2)"), -1, 40)) {
                org.argentumforge.engine.utils.ScreenshotUtils.takeScreenshot();
            }
            ImGui.popStyleColor(3);

            ImGui.dummy(0, 5);
            if (ImGui.button(getLabel("reset", "Restablecer Ajustes"), -1, 30)) {
                settings.resetPhotoMode();
            }
            ImGui.dummy(0, 5);
            if (ImGui.button(getLabel("exit", "Cerrar Modo Foto"), -1, 30)) {
